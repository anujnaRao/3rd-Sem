...
ounce = Button(top,text="Ounce",command = GConversion).grid(row = 1,column=2,ip$
...
pound = Button(top,text="Pounds",command = KConversion).grid(row = 2,column=2,i$


l3 = Label(top,text = "Ahr").grid(row = 3,column=0,padx=30,pady=20)
ahr = Entry(top,textvariable = val3).grid(row = 3,column = 1,ipadx = 10)

stone = Button(top,text="Stone",command = SConversion).grid(row = 3,column=2,ip$
...
bclear = Button(top,text="Clear",command = clear).grid(row=5,column=3,padx=20,p$
top.mainloop()
